...
def load_fasttext_embeddings(vocab, embedding_dim=300, fasttext_path="../wiki-news-300d-1M.vec"):
    ...
    with open(fasttext_path, 'r', encoding='utf-8', errors='ignore') as f:
        ...
        for line in tqdm(f, total=1000000):
            ...
            if word in vocab.stoi:
                ...
                found += 1
    print(f"Loaded {found} FastText vectors.")
...
fasttext_path = "../wiki-news-300d-1M.vec"  # download manually if missing
if os.path.exists(fasttext_path):
    fasttext_embeddings = load_fasttext_embeddings(dataset_full.vocab, embed_size, fasttext_path)
else:
    fasttext_embeddings = None
    print("FastText file not found. Using random embeddings.")

model = CNNtoLSTM(embed_size, hidden_size, vocab_size, embeddings=fasttext_embeddings).to(device)
...
for imgs, captions in tqdm(test_loader, desc="Evaluating"):
    ...
    for i in range(imgs.size(0)):
        ref_caption = [dataset_full.vocab.tokenizer_eng(df['caption'][i])]
        gen_caption = model.decoder.generate_caption(features[i], dataset_full.vocab)
...
for ref, hyp in zip(references, hypotheses):
    # NLTK expects list of references as list of token lists
    bleu_scores.append(sentence_bleu(ref, hyp, weights=(0.25,0.25,0.25,0.25), smoothing_function=smooth_fn))
